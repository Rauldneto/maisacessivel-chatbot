...
.b{max-width:82%;padding:10px 14px;border-radius:16px;font-size:14px;background:#fff;color:#222;border-bottom-left-radius:4px;align-self:flex-start;box-shadow:0 1px 4px rgba(0,0,0,.08)}
.u{max-width:82%;padding:10px 14px;border-radius:16px;font-size:14px;background:#FF6B00;color:#fff;border-bottom-right-radius:4px;align-self:flex-end}
.l{max-width:82%;padding:10px 14px;border-radius:16px;font-size:14px;background:#fff;color:#888;font-style:italic;align-self:flex-start}
...
function enviar(){
  ...
  .then(function(d){
    l.remove();
    var rep=d.reply||'Desculpe, tente novamente.';
    am(rep,'b');
    H.push({role:'assistant',content:rep});
  })
  .catch(function(){
    l.remove();
    am('Erro de conexao. Tente novamente.','b');
  });
}
